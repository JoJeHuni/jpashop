...
- 엔티티 조회
  - 엔티티를 조회해서 그대로 반환: V1
  - 엔티티 조회 후 DTO로 변환: V2 (여러 테이블을 사용하면 성능 X -> V3로 보완)
  - 페치 조인으로 쿼리 수 최적화: V3 (페치 조인은 페이징 불가능 -> V3.1로 보완)
  - 컬렉션 페이징과 한계 돌파: V3.1
    - 컬렉션은 페치 조인 시 페이징이 불가능
    - ToOne 관계는 페치 조인으로 쿼리 수 최적화
    - 컬렉션 (예제에서 Order의 OrderItem 같은 경우) 은 페치 조인 대신에 지연 로딩을 유지하고, `hibernate.default_batch_fetch_size` , `@BatchSize` 로 최적화
